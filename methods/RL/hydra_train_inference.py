...
#
# torch.cuda.set_device(0)  # Set the device explicitly
# dist.init_process_group(backend='nccl', rank=0, world_size=8)


class BlocksWorldTrainer:
    """Class for training and inference on blocksworld models using Hydra for configuration"""
    ...
    def inference(self):
        ...
        # Extract config values
        model_checkpoint = self.cfg.inference.checkpoint
        steps = self.cfg.inference.steps
        temperature = self.cfg.inference.temperature
        sc_num = self.cfg.inference.sc_num
        use_icl = self.cfg.inference.use_icl
        prompt_path = self.cfg.inference.prompt_path
        resume = self.cfg.inference.resume

        # Generate checkpoint path
        model_dir = self._get_checkpoint_path(model_checkpoint)

        # Setup data path
        data_path = self.cfg.inference.data_path.format(steps=steps)
        ...
        if use_icl:
            with open(self.cfg.experiment.icl_examples_file) as f:
                icl_examples = json.load(f)
            icl = generate_icl(icl_examples, provide_think_icl=True, num_icl=self.cfg.inference.icl_num)
        ...
        evaluator = BWEvaluator(
            config_file=self.cfg.inference.config_file,
            domain_file=self.cfg.inference.domain_file,
            data_path=data_path,
            init_prompt=prompt,
            disable_log=False,
        ...
        accuracy = evaluator.evaluate(
            reasoner,
            shuffle_prompt=True,
            num_shot=self.cfg.inference.num_shot,
            resume=resume,
            log_dir=log_dir
        )
